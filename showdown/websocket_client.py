...
class PSWebsocketClient:
    ...
    @classmethod
    async def create(cls, username, password, address, avatar, room):
    ...
    async def receive_message(self):
        ...
        if message.startswith(">" + self.room):
            msg = message.split("|")
            message = message.replace(msg[0], '')
        return message
    ...
    async def login(self):
        ...
        if response.status_code == 200:
            ...
            await self.send_message('', message)
            message = ["/j " + self.room]
            logger.debug("j'ai rejoin " + self.room)
            await self.send_message('', message)
        else:
